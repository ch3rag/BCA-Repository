...
*               Last Updated: September 9, 2020 09:25 PM               *
*                      File Name: Matrix DnQ.cpp                       *
*                         Source Language: cpp                         *
*       Repository: http://github.com/ch3rag/bca-repository.git        *
*                                                                      *
*                       --- Code Description ---                       *
*        Matrix Multiplication Using Divide & Conquer Technique        *
***********************************************************************/
...
int ** mulMat(int ** a, int ** b, int order) {
    if (order == 1) {
        ...
        combine(addMatrix(mulMat(divide(a, 0, 0, order / 2), divide(b, 0, 0, order / 2), order / 2),
			mulMat((divide(a, 0, 1, order / 2)), divide(b, 1, 0, order / 2), order / 2), order / 2),
			addMatrix(mulMat(divide(a, 0, 0, order / 2), divide(b, 0, 1, order / 2), order / 2),
			mulMat((divide(a, 0, 1, order / 2)), divide(b, 1, 1, order / 2), order / 2), order / 2), 
			addMatrix(mulMat(divide(a, 1, 0, order / 2), divide(b, 0, 0, order / 2), order / 2),
			mulMat((divide(a, 1, 1, order / 2)), divide(b, 1, 0, order / 2), order / 2), order / 2),
			addMatrix(mulMat(divide(a, 1, 0, order / 2), divide(b, 0, 1, order / 2), order / 2),
			mulMat((divide(a, 1, 1, order / 2)), divide(b, 1, 1, order / 2), order / 2), order / 2), order);
    }
}
